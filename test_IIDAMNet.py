...
# ==============================================================================
parser = argparse.ArgumentParser(description="Évaluation du modèle hybride II-DAMNet")
...
TEST_IMAGE_DIR = 'mixed_test_dataset/images'
TEST_GT_DIR    = 'rescue/DATA/test/labels'
...
class TestDataset4Classes(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. Obtenir le nom sans l'extension
        base_name = os.path.splitext(img_name)[0]
        
        # 2. NETTOYAGE DES PRÉFIXES
        clean_name = base_name
        if clean_name.startswith("day_"):
            clean_name = clean_name.replace("day_", "", 1)

        elif clean_name.startswith("kind_"):
            clean_name = clean_name.replace("kind_", "", 1)
        elif clean_name.startswith("trad_night_"):
            clean_name = clean_name.replace("trad_night_", "", 1)
        
        # 3. Construire le chemin vers le label
        gt_path = os.path.join(self.gt_dir, f"{clean_name}_lab.png") 
...
def main():
    ...
    # CORRECTION : Ajout du Resize avec l'interpolation NEAREST pour conserver l'intégrité des indices
    mask_transform = transforms.Compose([
        transforms.Resize((713, 713), interpolation=transforms.InterpolationMode.NEAREST),
    ...
    with torch.no_grad():
        ...
        for images, masks in pbar_test:
            ...
            # CORRECTION : Passage des masks pour correspondre au forward de votre architecture pendant le train
            ...
            # Filtrage des pixels valides (exclut l'ignore index 255)
    ...
    print("\n" + "="*50)
    print(" RÉSULTATS D'ÉVALUATION : TEST SET COHÉRENT")
    print("="*50)
